...
class LaneFinder:
    def __init__(self, img_size, warped_size, cam_matrix, dist_coeffs, transform_matrix, pixels_per_meter, warning_icon):
        ...
        self.dir = 0 # (left: -1, straight: 0, right: 1)
    ...
    def draw_lane_weighted(self, img, thickness=5, alpha=0.8, beta=1, gamma=0):
        ...
        #img += cv2.circle(img, self.img_size[1]-both_lines[1], 20, (0,0,255), -1) # right up
        #img = cv2.line(img, self.img_size[1]-both_lines[1], self.img_size[1]-both_lines[1]+(0, 1000), (0,0,255), 5)
        #img += cv2.circle(img, both_lines[1], 20, (0,0,0), -1) # right down
        #img = cv2.line(img, both_lines[1], both_lines[1]+(0, 1000), (0,0,0), 5)
        #img += cv2.circle(img, both_lines[-1], 20, (0,255,0), -1)
        #img += cv2.circle(img, self.img_size[1]-both_lines[-1], 20, (255,0,0), -1)


        ######################
        p_d = (both_lines[1]-(self.img_size[1]-both_lines[1]))
    
        if  (p_d[1]/p_d[0])< 2.1 and (p_d[1]/p_d[0]) > 1.9:
            self.dir = "Straight"

        elif p_d[1]/p_d[0] > 2.1:
            self.dir = "Right"
        ...
        if self.found:
            ...
            cv2.putText(img, "DIR: %s" % self.dir, (500, 50), cv2.FONT_HERSHEY_PLAIN, fontScale=3,
...
if __name__ == '__main__':
    ...
    for file in video_files:
        lf = LaneFinder(settings.ORIGINAL_SIZE, settings.UNWARPED_SIZE, cam_matrix, dist_coeffs,
                    perspective_transform, pixels_per_meter, "warning.png")
        output = os.path.join(output_path,"00lane_"+file)
