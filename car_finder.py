...
class DigitalFilter:
    ...
    def __init__(self, vector, b, a):
        ...
        self.speed_history_deq = deque([0 for i in range(25)])
    ...
    ##########################################
    def close_or_far(self):
        
        car_is_close = "Far"
        
        self.speed_history_deq.popleft()
        if self.speed() <= 0:
            self.speed_history_deq.append(1)
        else:
            self.speed_history_deq.append(0)

        check_speed = 0
        self.speed_history_deq.extend(self.speed_history_deq)
        for i in range(25):
            check_speed += self.speed_history_deq.popleft()
...
class Car:
    ...
    def draw(self, img, color=(0, 0, 255), thickness=3):
        if self.display:
            ...
            # far: RED
            # close: BLUE
            if self.position.close_or_far() == "Far":
                cv2.rectangle(img, (window[0], window[1]), (window[2], window[3]), color, thickness)
            else:
                cv2.rectangle(img, (window[0], window[1]), (window[2], window[3]), (255, 0, 0), thickness)
            if self.has_position:
                """
                ...
                # close: BLUE
                if self.position.close_or_far() == "Far":
                    cv2.putText(img, "Close or Far: %s"  % (self.position.close_or_far()), (int(window[0]), int(window[3]+20)),
                                cv2.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=3, color=(0, 0, 0))
                    cv2.putText(img, "Close or Far: %s"  % (self.position.close_or_far()), (int(window[0]), int(window[3]+20)),
                                cv2.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=2, color=(255, 255, 255))
                else: #close
                    cv2.putText(img, "Close or Far: %s"  % (self.position.close_or_far()), (int(window[0]), int(window[3]+20)),
                                cv2.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=3, color=(0, 0, 0))
                    cv2.putText(img, "Close or Far: %s"  % (self.position.close_or_far()), (int(window[0]), int(window[3]+20)),
                                cv2.FONT_HERSHEY_PLAIN, fontScale=1.25, thickness=2, color=(255, 0, 0))               
                ##########################################
...
class CarFinder:
    ...
    def find_cars(self, img, threshold = 2, reset = False):
        ...
        for lbl in range(labels):
            ...
            near = cv2.inRange(car_img, medi - np.array([40, 40, 40]),medi + np.array([40, 40, 40]))
            if near is not None:
...
if __name__ == '__main__':
    ...
    #video_files = ['test2.mp4', 'test3.mp4']
    #video_files = ['test_video.mp4','project_video.mp4','project_video-1.mp4']
...
# 1: 원본
# 2: 거리 계산을 통해 매 프레임마다 멀어지는지 가까워지는지 far, close를 구분
# 3: far, close의 임계값 설정 (10)
# 4: far, close만 남겨놓음, 방향 미적용
# 5: close인 경우 RED 색상 변경, Far and Close queue로 구현, 방향 적용
